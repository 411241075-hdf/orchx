...
>
> **macOS Sequoia (14.x/15.x) + Python 3.14 нюанс.** pip-созданные
> `.pth` файлы получают флаг `UF_HIDDEN` (через `com.apple.provenance`),
> и Python 3.14 их пропускает (`site.py:185`, "Skipping hidden .pth file").
> В итоге `import orchx` падает. `make install` автоматически снимает
> флаг (target `_fix-macos-hidden-pth`). Если ставите вручную:
