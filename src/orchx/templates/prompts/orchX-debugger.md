...
2. **Воспроизведи провал.** Если в acceptance есть shell-команда — запусти её через `bash`. Зафиксируй точный вывод. Если acceptance — `file_exists` или `file_contains` — `read` файл, проверь.
   ...
   **🚨 Shared-file дисциплина при reimplement.** Если для починки нужно
   ...
   `backend/api/<x>/​__init__.py`, `frontend/src/App.jsx`, корневой
   `pyproject.toml`), а сам файл в worktree ОТСУТСТВУЕТ — НЕ создавай
   его с нуля по шаблону из памяти или из соседнего worktree.

   Между запуском оригинального воркера и твоим retry'ем в integration
   ветку могли вмерджиться СОСЕДНИЕ задачи, которые ДОБАВИЛИ в этот файл
   свои import'ы / `include_router(...)` / экспорты. Если ты создашь
   файл «с нуля» — ты тихо снимёшь их регистрации, и после merge
   /api/X/* станет 404 (см. `api-admin-db` в .orchx/runs/admin-subdomain
   как живой пример провала).

   Безопасный путь:
   - Прочитай секцию `<integration_branch_state>` в task.md — там список
     уже смержённых соседних задач с их goal'ами.
   - Если затронутый shared-файл должен содержать чужие регистрации
     (по списку выше), но в worktree отсутствует — отметь это в
     `notes` как блокер чек-аута и поставь `status: "failed"`. Диспетчер
     пересоздаст worktree от свежего ref'а на следующем retry'е.
   - Никогда не «восстанавливай» shared-файл из памяти/template'а,
     даже если знаешь его структуру.
...
`python -c "import re; print(re.search(...))"` работает напрямую,
не нужно создавать helper-файл. Композитные команды снаружи
кавычек (`cmd1 && cmd2`, `cat x | grep y`) по-прежнему блокируются.
