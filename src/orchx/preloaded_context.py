"""Предзагрузка фрагментов кода в task.md.

Воркеры в orchX-worktree запускаются с холодным контекстом. На больших
файлах каждый воркер заново делает 3-5 grep'ов, чтобы найти уже известное
